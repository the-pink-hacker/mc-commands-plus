...
public class ClearSpawnPointCommand {
    ...
    private static int clearSpawnPoint(ServerCommandSource source, Collection<ServerPlayerEntity> players) throws CommandSyntaxException {
        ...
        if (i > 0) {
            source.sendFeedback(Text.translatable("command.clearspawnpoint.success", i), true);
        } else {
            throw new SimpleCommandExceptionType(Text.translatable("command.clearspawnpoint.fail")).create();
        }
